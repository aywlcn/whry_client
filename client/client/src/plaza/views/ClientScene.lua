...
--    self:addChild(par)
	...
	btn = getChildFormObject(areaTop , "btn_exchange") -- areaTop:getChildByName("btn_exchange")
    ...
    -- add by wss
    self.m_quickStart:setVisible(false)
	...
	btn:addTouchEventListener(btcallback)]]
	--商城
	btn = getChildFormObject(areaBottom , "btn_shop") -- areaBottom:getChildByName("btn_shop")
	btn:setTag(ClientScene.BT_SHOP)
	btn:addTouchEventListener(btcallback)
	--好友
	btn = getChildFormObject(areaBottom , "btn_friend")  -- areaBottom:getChildByName("btn_friend")
	btn:setTag(ClientScene.BT_FRIEND)
	btn:addTouchEventListener(btcallback)
	self.m_btnFriend = btn
	--任务	
	btn = getChildFormObject(areaBottom , "btn_task") -- areaBottom:getChildByName("btn_task")
	...
	btn = getChildFormObject(self._notify , "btn_trumpet") -- self._notify:getChildByName("btn_trumpet")
...
--	self._notifyClip:addTo(self._notify)
    ...
    local stencil  = display.newSprite()
		:setAnchorPoint(cc.p(0,0.5))
	stencil:setTextureRect(cc.rect(0,0,self._notify:getContentSize().width ,self._notify:getContentSize().height))
	self._notifyClip = cc.ClippingNode:create(stencil)
		:setAnchorPoint(cc.p(0,0.5))
	self._notifyClip:setInverted(false)
	self._notifyClip:move( self._notify:getPositionX() - self._notify:getContentSize().width/2 , self._notify:getPositionY() )
	self._notifyClip:addTo(self._notify)
