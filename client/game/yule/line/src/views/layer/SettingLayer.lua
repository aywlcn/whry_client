...
function SettingLayer:ctor( verstr )
    ...
    --加载csb资源
    self._csbNode = ExternalFun.loadCSB("setting/setLayer.csb", self)
    ...
    if GlobalUserItem.bVoiceAble == true then 
        self.m_btnMusic:loadTextureNormal("setting/btn_set_open.png")
    else
        self.m_btnMusic:loadTextureNormal("setting/btn_set_close.png")
    end
    if GlobalUserItem.bSoundAble == true then 
        self.m_btnEffect:loadTextureNormal("setting/btn_set_open.png")
    else
        self.m_btnEffect:loadTextureNormal("setting/btn_set_close.png")
    end

    -- 帮助
    btn = self._csbNode:getChildByName("Button_rule")
    btn:setTag(SettingLayer.RULE)
    btn:addTouchEventListener(function (ref, eventType)
        if eventType == ccui.TouchEventType.ended then
            ExternalFun.playClickEffect()
            self.m_parent:getParentNode():getParentNode():popHelpLayer2(508, 0, yl.ZORDER.Z_HELP_BUTTON)
        end
    end)
...
function SettingLayer:OnButtonClickedEvent( tag, sender )
    if SettingLayer.BT_MUSIC == tag then    --音乐
        ...
        if GlobalUserItem.bVoiceAble == true then 
            ...
            sender:loadTextureNormal("setting/btn_set_open.png")
        else
            AudioEngine.stopMusic()
            sender:loadTextureNormal("setting/btn_set_close.png")
        end
