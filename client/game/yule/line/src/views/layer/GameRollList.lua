...
function GameRollList:update( time)
    ...
    if self.m_Status == LIST_STATE.RollNormall_Status then
        for index=1,2 do
            self.m_RollLayout[index]:setPositionY(self.m_RollLayout[index]:getPositionY() - self.MoveSpeed);
    ...
    if self.m_Status == LIST_STATE.SlowDown_Status then
        ...
        if self.m_slowdown_distant < self.MinSpeed*self.ListSlowTimes then
            times = self.ListSlowTimes/2;
        end

        local movespeedTemp = self.m_slowdown_distant/self.ListSlowTimes;
        if movespeedTemp > self.MoveSpeed then
            movespeedTemp = self.MoveSpeed;
        end
        if movespeedTemp < self.MinSpeed then
            movespeedTemp = self.MinSpeed
        end
    ...
    if self.m_Status == LIST_STATE.SlowDown_Status then
        ...
        for index=1,3 do
            ...
            if cc.rectContainsPoint(rect,cc.p(pointx,pointy)) or cc.rectContainsPoint(rect,pointtop) then
                ...
                self.m_slowdown_distant = pointy+self.ListHeight*4;
