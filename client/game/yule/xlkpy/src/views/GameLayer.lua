...
local QueryDialog = appdf.req("app.views.layer.QueryDialog")
...
local CannonLayer = module_pre .. ".views.layer.CannonLayer1"
...
function GameLayer:onQueryExitGame()
    ...
    self._queryDialog = QueryDialog:create("您要退出游戏么？", function(ok)
        if ok == true then
            ...
            self._gameView.m_touchTime = currentTime()
            self._gameView:hideBtnSchedule()
        end
