...
function Cannon:setMultiple(multiple)
    ...
    if nMultipleValue / self.parent._dataModel.m_secene.MaxShoot >= 0 and nMultipleValue / self.parent._dataModel.m_secene.MaxShoot < 0.4 then
        bulletNum = 1
    elseif nMultipleValue / self.parent._dataModel.m_secene.MaxShoot >= 0.4 and nMultipleValue / self.parent._dataModel.m_secene.MaxShoot < 0.7  then
        bulletNum = 2
    elseif nMultipleValue / self.parent._dataModel.m_secene.MaxShoot >= 0.7 and nMultipleValue / self.parent._dataModel.m_secene.MaxShoot <= 1  then
        bulletNum = 3
    end
...
function Cannon:productBullet( isSelf,fishIndex, netColor,fire)
	...
	if isSelf then
        ...
        if self.m_nChairID <3 or self.m_nChairID ==7 then
            final_angle = final_angle-180
        elseif self.m_nChairID ==6 then
...
function Cannon:setCannonType( cannontype,times)	
	...
	if cannontype == Game_CMD.CannonType.Special_Cannon then
		...
		local nBulletNum = 1 --math.floor(self.m_nMutipleIndex/4) + 1
        if self.m_nMutipleIndex / self.parent._dataModel.m_secene.MaxShoot >= 0 and self.m_nMutipleIndex / self.parent._dataModel.m_secene.MaxShoot < 0.4 then
            nBulletNum = 1
        elseif self.m_nMutipleIndex / self.parent._dataModel.m_secene.MaxShoot >= 0.4 and self.m_nMutipleIndex / self.parent._dataModel.m_secene.MaxShoot < 0.7  then
            nBulletNum = 2
        elseif self.m_nMutipleIndex / self.parent._dataModel.m_secene.MaxShoot >= 0.7 and self.m_nMutipleIndex / self.parent._dataModel.m_secene.MaxShoot <= 1  then
            nBulletNum = 3
        end
...
function Cannon:autoUpdate(dt)
	...
	if score < self.parent.parent.CurrShoot[1][self.m_nChairID+1] then
		...
		if nil == self._queryDialog then
      		local this = self
	    	self._queryDialog = QueryDialog:create("上分不足,请充值", function(ok)
...
function Cannon:otherUpdate(dt)
    ...
    if fire.chair_id <3 or fire.chair_id ==7 then
       angle = angle+180
    elseif fire.chair_id ==6 then
       angle = angle+90
    end
	angle = angle*57.32
    local ifchange=false
    if self.m_nChairID>2 then
       if fire.chair_id>2 then            --4,5,6同边位置
          ifchange=true
       else
          ifchange=false
       end
    else
       if fire.chair_id<3 then            --1,2,3同边位置
          ifchange=true
       else
          ifchange=false
       end
