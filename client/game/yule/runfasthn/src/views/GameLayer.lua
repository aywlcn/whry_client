...
function GameLayer:OnEventGameClockInfo(chair,time,clockId)
    ...
    if clockId == cmd.IDI_OUT_CARD then --出牌时间
    	if (time == 0 or self.bTrustee[cmd.MY_VIEWID] and time < cmd.TIME_OUT_CARD) and chair == self:GetMeChairID() then
    		if self.bTrustee[cmd.MY_VIEWID] == false then
    			--超时加1
...
function GameLayer:sendAutomatism()
	...
	cmd_data:pushbool(not self.bTrustee[cmd.MY_VIEWID])
