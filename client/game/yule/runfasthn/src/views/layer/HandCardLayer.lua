...
function HandCardLayer:ctor(scene)
	...
	for i = 1, 3 do
		self.nodeCard[i] = cc.Node:create()
			:move(posCard[i])
			:setContentSize(i == cmd.MY_VIEWID and cc.size(width, 194) or cc.size(143, height))
...
function HandCardLayer:setHandCard(cardData, cardCount)
	...
	if cardCount[cmd.MY_VIEWID] < 16 then
		for i = cardCount[cmd.MY_VIEWID] + 1, 16 do
			self.nodeCard[cmd.MY_VIEWID]:getChildByTag(i):setVisible(false)
		end
	end
end
...
function HandCardLayer:onTouchEnded(touch, event)
	...
	local nodeX, nodeY = self.nodeCard[cmd.MY_VIEWID]:getPosition()
	local nodeSize = self.nodeCard[cmd.MY_VIEWID]:getContentSize()
		...
		if card and not self.bCardOut[i] then
			...
			if cc.rectContainsPoint(rectCard, posTemp) then
				...
				for j = i + 1, #self.cbCardData do
					local card1 = self.nodeCard[cmd.MY_VIEWID]:getChildByTag(j)
...
function HandCardLayer:updateCard(viewId)
	...
	self.nodeCard[viewId]:setContentSize(viewId == cmd.MY_VIEWID and cc.size(width, 194) or cc.size(143, height))
