...
function SetLayer:ctor(scene)
	...
	local btnHelp = self._csbNode:getChildByName("bt_help")
		:setTag(TAG_BT_HELP)
	btnHelp:addClickEventListener(funCallback)
	--声音
...
function SetLayer:onButtonCallback(tag, ref)
	...
	elseif tag == TAG_BT_EXIT then
		print("离开")
		self:hideLayer()
	elseif tag == TAG_BT_HELP then
		print("帮助")
		self:hideLayer()
		self._scene._scene._scene:popHelpLayer2(cmd.KIND_ID, 0, yl.ZORDER.Z_HELP_WEBVIEW)
	end
end
