...
local SetLayer = appdf.req(appdf.GAME_SRC.."yule.runfasthn.src.views.layer.SetLayer")
...
function GameViewLayer:onExit()
	...
	cc.SpriteFrameCache:getInstance():removeSpriteFramesFromFile("game/gameLayer.plist")
	cc.Director:getInstance():getTextureCache():removeTextureForKey("game/gameLayer.png")
	cc.SpriteFrameCache:getInstance():removeSpriteFramesFromFile("result/resultLayer.plist")
	cc.Director:getInstance():getTextureCache():removeTextureForKey("result/resultLayer.png")
...
function GameViewLayer:ctor(scene)
	...
	local btnSet = self._csbNode:getChildByName("Button_set")
	...
	self.nodePlayer[cmd.MY_VIEWID]:getChildByName("Text_remainCard"):move(-34, 81)
	self.nodePlayer[cmd.MY_VIEWID]:getChildByName("Sprite_ready"):move(500, 75)
	self.nodePlayer[cmd.MY_VIEWID]:getChildByName("Sprite_trustee"):move(-34, 25)
	self.nodePlayer[cmd.MY_VIEWID]:getChildByName("Text_bomb"):move(170, 17)
...
function GameViewLayer:manageCardType(viewId, cbCardData)
	...
	elseif cbCardType == cmd.CT_THREE_LINE_TAKE_SINGLE then --三带单
		if #cbCardData == 4 then
			...
			self._scene:PlaySound(str.."THREE_ONE_LINE.wav")
		end
