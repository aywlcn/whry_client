   ...
   if nfishtype == g_var(cmd).FISH_YUAN_BAO then
    ...
    local ptext = cc.LabelAtlas:create(string.format("%d", score),"game_res/num_game_gold.png",37,34,string.byte("0"))
     ...
     for i=1,secondNum do
       ...
       local pscale = 1
       if nfishtype == g_var(cmd).FISH_YUAN_BAO then
            pcoin = cc.Sprite:create("game_res/im_yb_end.png",cc.rect(0, 0, 164,185))
        else
 ...
 local fishName = {"小黄鱼","小蓝鱼","小丑鱼","丝鲈鱼","神仙鱼","河豚鱼","灯笼鱼","八爪鱼","海龟","水母","剑鱼","魔鬼鱼","海豚","鲨鱼","蓝鲸","银鲸","金鲸","美人鱼"
  ...
  if data.nFishMultiple >= 50 then
    ...
    elseif data.nScoreType == g_var(cmd).EST_Kill then
       tipStr = "击中炸弹鱼，获得"
    elseif data.nScoreType == g_var(cmd).EST_YuanBao then
       tipStr = "击中元宝，获得"
    end
...
function GameViewLayer:showBomb(cannon)
    local position = cc.p(cannon:getPositionX(),cannon:getPositionY())
    math.randomseed(os.time())
    local call = cc.CallFunc:create(function()
        local positionx = math.random(-150, 150)
        local positiony = math.random(-150, 150)
        local pbomb = cc.Sprite:createWithSpriteFrameName("bomb_0.png")
        pbomb:setPosition(cc.p(position.x + positionx, position.y + positiony))
        local panim = cc.AnimationCache:getInstance():getAnimation(g_var(cmd).BombAnim)
        pbomb:runAction(cc.Animate:create(panim))
        self:addChild(pbomb, self._scene.fishLayerZorder)
        end)
    self:runAction(cc.Repeat:create(cc.Sequence:create(call, cc.DelayTime:create(0.048)), 8 ))
    self._scene._dataModel:playEffect(g_var(cmd).Combo)
end
