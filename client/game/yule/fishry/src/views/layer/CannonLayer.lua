...
function CannonLayer:initUserInfo(viewpos,userItem)
	...
	score:setString(string.format("%d", self._dataModel.m_secene.lPlayCurScore[1][userItem.wChairID+1])) 
...
function CannonLayer:updateUserIngot( ingot,cannonpos )
	...
	local ingotLB = infoBG:getChildByTag(TAG.Tag_userIngot)
	if ingot >= 0 and nil ~= ingotLB then
		ingotLB:setString(string.format("元宝:%d", ingot))
	end
...
function CannonLayer:showCannonByChair( pos,chair )
	...
	if nil ~= wChairID then
		local tMultipleValue = self._dataModel.m_secene.nMultipleValue[1][self._dataModel.m_secene.nMultipleIndex + 1]
