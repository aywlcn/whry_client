...
function Cannon:productBullet( isSelf,fishIndex, netColor)
	...
	--print("------------productBullet m_fishCount------------",self._dataModel.m_fishCount)
	if self._dataModel.m_fishCount <= 0 then
		--return
	end
	...
	bullet1:setTag(g_var(cmd).Tag_Bullet)
...
function Cannon:ShowSupply( data )
	...
	if nSupplyType == g_var(cmd).EST_Laser then
...
function Cannon:autoUpdate(dt)
	...
	print("autoUpdate m_fishCount ",self._dataModel.m_fishCount)
	if self._dataModel.m_fishCount <= 0 then
		--return
	end
