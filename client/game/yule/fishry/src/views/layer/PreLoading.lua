...
function PreLoading.readAniByFileName( file,width,height,rownum,linenum,savename,time)
	local frames = {}
	time = time or 0.03
	for i=1,rownum do
		for j=1,linenum do
...
function PreLoading.readAniams()
	...
	for index=0,6 do
   	...
   	PreLoading.readAnimation("fort_light_", g_var(cmd).FortLightAnim, 6, 0.02,1);
    	...
    	PreLoading.readAniByFileName("game_res/im_yb_fish.png", 239, 226, 2, 1, g_var(cmd).YBFish,0.12)
    	PreLoading.readAniByFileName("game_res/im_yb_die.png", 215, 245, 2, 4, g_var(cmd).YBDie,0.12)
    	PreLoading.readAniByFileName("game_res/im_yb_end.png", 164, 185, 3, 5, g_var(cmd).YBAnim,0.08)
