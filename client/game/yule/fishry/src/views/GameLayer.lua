  ...
  local function dealCanAddFish()
    ...
    for i = 1, #self._dataModel.m_waitList do
      ...
      if iscanadd then
          ...
          if fishdata.nFishType == g_var(cmd).FISH_YUAN_BAO then
            print(" ============ isCanAddtoScene fishType "..fishdata.nFishType .. " key ".. fishdata.nFishKey .. " =============== ")
          end
          self._dataModel.m_fishList[fish.m_data.nFishKey] = fish
          self._dataModel.m_fishCount = self._dataModel.m_fishCount + 1
          if fish.m_data.nFishType ==  g_var(cmd).FISH_XIANG_ZI and fish.m_data.wHitChair == self.m_nChairID then
...
function GameLayer:onEventGameMessage(sub,dataBuffer)
  ...
  elseif sub == g_var(cmd).SUB_S_FISH_CATCH	and true == PRELOAD.bLoadingFinish and false == self.m_bLeaveGame then
  --捕获鱼
...
function GameLayer:onSubUpdateGame( databuffer )
  ...
  self._dataModel.m_secene.nBulletVelocity[1][g_var(cmd).BULLET_MAX] = update.nBulletVelocity
  self._dataModel.m_secene.nBulletCoolingTime[1][g_var(cmd).BULLET_MAX] = update.nBulletCoolingTime
  self._dataModel.m_secene.nFishMultiple = update.nFishMultiple
  self._dataModel.m_secene.nMultipleValue = update.nMultipleValue
  self._dataModel.m_secene.bUnlimitedRebound = update.bUnlimitedRebound
end
...
function GameLayer:onSubSupply(databuffer )
  ...
  if supply.nSupplyType == g_var(cmd).EST_Gift then
  ...
  local tipStr = ""
   ...
   --self._gameView:showBomb(cannon)
  ...
  elseif supply.nSupplyType == g_var(cmd).EST_Speed then
      tipStr = userItem.szNickName.." 击中补给箱打出了加速！所有子弹速度翻倍！"
  elseif supply.nSupplyType == g_var(cmd).EST_Strong then
      tipStr = userItem.szNickName.." 击中补给箱打出了强化！所有鱼网范围扩大！"
      self._dataModel:playEffect(g_var(cmd).SHELL_7)
  elseif supply.nSupplyType == g_var(cmd).EST_Bomb then
      tipStr = userItem.szNickName.." 击中补给箱打出了深水炸弹！轰! ! ! ！"
      self._gameView:showBomb(cannon)
...
function GameLayer:onSubFishCreate( dataBuffer )
    ...
    if fishNum >= 1 then
    	for i=1,fishNum do
         ...
         if FishCreate.nFishType == g_var(cmd).FISH_XIANG_ZI then
            print("=================FishCreate FISH_XIANG_ZI ================",count)
          end
          count = count + 1
...
--[[
function GameLayer:onSubFire2(databuffer)
  if not self.m_cannonLayer   then
    return
  end
...
function GameLayer:onSubFishCatch( databuffer )
      ...
      if catchNum >= 1 then
		...
		while true do
           ...
           if catchData.nScoreType ~= g_var(cmd).EST_NULL then
              dump(catchData, "============== onSubFishCatch ================", 6)
           end
           ...
           if nil ~= fish and nil ~= fish.m_data then
            ...
            if fish.m_data.bKiller and false == showLight then
              print("--------- showLight ----------")
              showLight = true
              self._gameView:showLight(fish.m_data.nFishType,fish.m_data.nFishKey)
            end
           ...
           if catchData.wChairID == self.m_nChairID then   --自己
                ...
                print("========== others lPlayCurScore ==========",catchData.nScoreType)
                if catchData.nScoreType == g_var(cmd).EST_Cold then
                    ...
                    print("========== others lPlayCurScore ==========",self._dataModel.m_secene.lPlayCurScore[1][catchData.wChairID + 1],catchData.lScoreCount)
                end
                  --获取用户
                  local userid = self.m_cannonLayer:getUserIDByCannon(cannonPos+1)
