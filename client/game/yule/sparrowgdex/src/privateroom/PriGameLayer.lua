...
function PriGameLayer:onButtonClickedEvent( tag, sender )
    ...
    elseif BTN_INVITE == tag then
        PriRoom:getInstance():getPlazaScene():popTargetShare(function(target, bMyFriend)
            ...
            local shareTxt = "广东麻将约战 房间ID:" .. self.m_atlasRoomID:getString() .. " 局数:" .. PriRoom:getInstance().m_tabPriData.dwDrawCountLimit
            local friendC = "广东麻将房间ID:" .. self.m_atlasRoomID:getString() .. " 局数:" .. PriRoom:getInstance().m_tabPriData.dwDrawCountLimit
            ...
            elseif nil ~= target then
                ...
                MultiPlatform:getInstance():shareToTarget(target, sharecall, "广东麻将约战", shareTxt .. " 广东麻将精彩刺激, 一起来玩吧! ", url, "")
            end
        end)
