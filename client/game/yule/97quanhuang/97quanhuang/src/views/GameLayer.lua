...
function GameLayer:OnResetGameEngine()
    ...
    if (self.m_bIsAuto == true and useritem.cbUserStatus == yl.US_PLAYING and self.m_cbGameStatus == 101) or(self.m_cbGameStatus == 102 and self.m_bEnterGame2 == false) then
...
function GameLayer:onEventGameSceneFree(buffer)
    ...
    -- 获得网络信息回馈 ExternalFun.read_netdata qweqwe
    -- 初始化数据
...
function GameLayer:onEventGameMessage(sub, dataBuffer)
    ...
    elseif sub == g_var(cmd).SUB_S_THREE_START then
        -- 小玛丽开始
        -- print("watermargin 小玛丽开始")
        self:onGame3Start(dataBuffer)
    elseif sub == g_var(cmd).SUB_S_THREE_KAI_JIANG then
        -- 小玛丽开奖
    ...
    elseif sub == g_var(cmd).SUB_S_USER_WIN_JIANGJIN then
        -- 小游戏获得积分
        print("拳皇 ---》 奖金池大派奖积分")
...
function GameLayer:Show_JiangJinChiDaPaiJiang(dataBuffer)
    ...
    local cmd_table = ExternalFun.read_netdata(g_var(cmd).SHZ_CMD_S_SendJiang_Score, dataBuffer)
    -- 获取网络回馈奖金池
...
function GameLayer:onJIANGJINCHIGetScore(dataBuffer)
    -- 游戏开始
    local cmd_table = ExternalFun.read_netdata(g_var(cmd).SHZ_CMD_S_JiangJin_Score, dataBuffer)
    -- 获取网络回馈奖金池
...
function GameLayer:refreshScore()
    local useritem = self:GetMeUserItem()
    self._gameView.m_textScore = useritem.lScore
    self.m_lCoins = self:GetMeUserItem().lScore
...
--    --设置压分label的数值
--    self._game2View.m_textYafen = self.m_lYafen2
--    --设置用户分数
--    self.m_lCoins = self.m_lCoins + self.m_lYafen2
--    self._game2View.m_textScore = self.m_lCoins
--    --不能点击按钮
...
--     self._gameView:SetNumber_And_Meney(1,self._gameView.m_textScore )       -- 1 ：玩家金币
--    -- if self.m_bIsAuto == true then
--    --     self:onGameStart()
...
function GameLayer:onSubGetScore(dataBuffer)
    print("小游戏结算当前操作积分")
    local cmd_table = ExternalFun.read_netdata(g_var(cmd).SHZ_CMD_S_Three_Res_Score, dataBuffer)
    -- dump(cmd_table)
