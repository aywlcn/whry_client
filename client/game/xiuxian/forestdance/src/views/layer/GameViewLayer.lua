...
function GameViewLayer:blinkEffect(isAction,index)
  local circles = {}
  for i=1,6 do
...
function GameViewLayer:updateColor()
   ...
   local colorList = self._scene._gameModel._sceneData.cbColorLightIndexList[1]
...
function GameViewLayer:runRotateAction(dt)
  ...
  self._scene._rotateTime = self._scene._rotateTime + dt
  ...
  if self._scene._rotateStatus == g_var(cmd).Speed then
    self._scene._rotateSpeed = self._scene._rotateSpeed + dt*0.8
  ...
  elseif self._scene._rotateStatus == g_var(cmd).ConSpeed then
     if self._scene._rotateTime > 8 then
       ...
       local angle0 = 360 - self._scene._drawData.nAnimalAngle*15
  ...
  elseif self._scene._rotateStatus == g_var(cmd).SlowDown then
     self._scene._rotateSpeed = self._scene._rotateSpeed - dt*1.1
  ...
  elseif self._scene._rotateStatus == g_var(cmd).RightJust then
    local angle = self._animLayer:getRotation3D().y
    local stopAngle = 360 - self._scene._drawData.nAnimalAngle*15
    local _angle =  angle - stopAngle
    if math.abs(_angle) < g_var(cmd).RotateMin then
...
function GameViewLayer:rotateEnd()
  ...
  elseif  0 ~= bit:_and(self._scene._drawData.dwAnimationFlag,g_var(cmd).ANIMATION_PAYOUTDSX) then
    ...
    return 
      ...
      self._scene._drawIndex = index + 1

      for i,v in ipairs(self._animals) do
        local animal = v
        if animal:getTag() == self._scene._drawIndex then
          ...
          local soundIndex = resType*3 + self._scene._drawData.cbPointerColor
      ...
      self._camera:runAction(cc.MoveTo:create(0.5, g_var(cmd).Camera_Win_Vec3))
  end
end
...
function GameViewLayer:setGameStatus(status)
  ...
  self:updateClockType(self._scene._gameStatus-g_var(cmd).IDI_GAME_FREE)
end
...
function GameViewLayer:updateClockType(clockType)
  ...
  if 0 == clockType then
    self._scene:SetGameClock(0,0,self._scene._gameModel._sceneData.cbFreeTimeCount)
    clockTypeIMG:loadTexture("biaoti7.png",UI_TEX_TYPE_PLIST)
  elseif 1 == clockType then
    self._scene:SetGameClock(0,0,self._scene._gameModel._sceneData.cbBetTimeCount)
  ...
  elseif 3 == clockType then
    self._scene:SetGameClock(0,0,self._scene._gameModel._sceneData.cbPayOutTimeCount)
...
function GameViewLayer:showDrawResult( result )
  ...
  local winColor = self._rewardLayer:getChildByName("winType")  --中奖颜色
  local value = bit:_or(result.dwDrawBetItem,result.dwDrawWeaveItem)
  self:getResultInfo(value,winColor) 
...
function GameViewLayer:getResultInfo(routeData,target)
...
--插入路单记录
function GameViewLayer:insertRecord(oneRecord)
