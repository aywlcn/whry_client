...
local popPosition = {}
popPosition[cmd.LEFT_VIEWID] = cc.p(190, 254)
popPosition[cmd.RIGHT_VIEWID] = cc.p(772, 254)
popPosition[cmd.MY_VIEWID] = cc.p(85, 100)
local popAnchor = {} 
...
function GameRoleItem:switeGameState( isBanker )
    --if self.m_nViewId ~= cmd.MY_VIEWID then
        ...
        if nil == self.m_clipScore then
            self.m_clipScore = ClipText:createClipText(cc.size(100, 20), self.m_userItem.lScore .. "")
            self.m_clipScore:setAnchorPoint(cc.p(0.5, 0.5))
            self.m_clipScore:setPosition(80, 15.5)
...
function GameRoleItem:switeRoomerState( isRoomer )
    self.m_spRoomerFlag:setVisible(isBanker)
end
