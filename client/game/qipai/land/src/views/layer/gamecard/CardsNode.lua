...
local TAB_Y_POS = 
{
	150, 135, 120, 105, 90, 75, 60, 45, 30, 15, 0, -15, -30, -45, -60, -75, -90, -105, -120, -135
}
		...
		if not self.m_bShowCard then
			...
			for i = 1, self.m_cardsCount do
				...
				if nil ~= tmp then
					...
					tmp:stopAllActions()
					tmp:runAction(cc.Sequence:create(cc.DelayTime:create(i * 0.01) , spa))
				...
				if nil ~= tmp then
					...
					if tmp:getDispatched() then
						...
						local showBack = cc.CallFunc:create(function ()
							...
							--变动通知
							if nil ~= self.m_pSelectedListener and nil ~= self.m_pSelectedListener.onCountChange then
								self.m_pSelectedListener:onCountChange( nShowCount, self )
							end
							nShowCount = nShowCount + 1
						end)
