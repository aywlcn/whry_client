...
function GameLayer:onSubPassCard(dataBuffer)
    ...
    if self:IsValidViewID(curView) and self:IsValidViewID(passView) then
        ...
        if curView == cmd.MY_VIEWID then
            --[[local sel = self._gameView.m_tabNodeCards[cmd.MY_VIEWID]:getSelectCards()
            -- 提示回位
            if self._gameView.m_tabNodeCards[cmd.MY_VIEWID].m_bSuggested then 
                self._gameView.m_tabNodeCards[cmd.MY_VIEWID]:suggestShootCards(sel)
            else
                self._gameView.m_tabNodeCards[cmd.MY_VIEWID]:backShootedCards(sel)
            end]]
            self._gameView:onGetOutCard(curView, curView, self.m_tabLastCards)
        else
            self._gameView:onGetOutCard(curView, curView, {})
        end
...
function GameLayer:onSubGameConclude(dataBuffer)
    ...
    self._gameView:runAction(cc.Sequence:create(cc.DelayTime:create(1.0), cc.CallFunc:create(function()
        ...
        for k, v in pairs(handCards) do
            self._gameView.m_tabNodeCards[k]:showLeftCards(v)
        end
        -- 标志
        for i = 1, 3 do
            local chair = i - 1
